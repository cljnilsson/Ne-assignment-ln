...
function App() {
	...
	async function onInit() {
		const userInfo = await getRequest<User>("/api/user/info");
		...
		const booksInfo = await getRequest<{ books: Book[] }>("/api/books");
		console.log(booksInfo);
		if (booksInfo && booksInfo.success && booksInfo.data) {
						...
						element={
							<>
								...
								<div className="row mt-5">
									<div className="col">
										<p>
											Ultra primitive cart simulation (no
											way I am doing it properly in the
											time limit), buy hardcoded items
										</p>
										<button
